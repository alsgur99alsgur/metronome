...
export function openDataWindow(node, nodeResult, viewerWindow = null) {
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    body { margin: 0; color: #111827; background: #f8fafc; font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; }
    header { min-height: 58px; display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 10px 14px; border-bottom: 1px solid #d7dce2; background: #fff; }
    h1 { margin: 0; font-size: 16px; }
    .sub { color: #64748b; font-size: 12px; margin-top: 3px; }
    .tools { display: flex; align-items: center; gap: 8px; }
    input, select { height: 32px; border: 1px solid #cbd5e1; border-radius: 6px; background: #fff; padding: 0 8px; }
    #global-search { width: 260px; }
    button { height: 32px; border: 1px solid #cbd5e1; border-radius: 6px; background: #fff; color: #111827; cursor: pointer; font-weight: 700; padding: 0 10px; }
    main { height: calc(100vh - 58px); display: grid; grid-template-columns: 230px 1fr; min-height: 0; }
    aside { border-right: 1px solid #d7dce2; background: #fff; padding: 12px; overflow: auto; }
    .column-title { margin-bottom: 10px; color: #475569; font-size: 12px; font-weight: 800; text-transform: uppercase; }
    .column-toggle { display: grid; grid-template-columns: 18px 1fr; align-items: center; gap: 8px; min-height: 28px; font-size: 13px; text-align: left; }
    .column-toggle input { width: 14px; height: 14px; margin: 0; justify-self: start; }
    ...
    th, td { border-right: 1px solid #eef2f7; border-bottom: 1px solid #eef2f7; padding: 8px 10px; text-align: left; white-space: nowrap; }
    th { position: sticky; top: 0; z-index: 2; background: #f1f5f9; vertical-align: top; font-size: 12px; font-weight: 800; user-select: none; }
    .row-number-header, .row-number-cell { width: 64px; min-width: 64px; max-width: 64px; text-align: right; color: #64748b; }
    ...
    .sort-label { cursor: pointer; display: block; margin-bottom: 6px; }
    .filter-row { display: grid; grid-template-columns: minmax(92px, auto) minmax(120px, 1fr); gap: 6px; align-items: center; }
    .filter-row select, .filter-row input { width: 100%; height: 28px; font-size: 12px; }
    ...
    td.empty-cell { padding: 24px; color: #64748b; text-align: center; background: #fff; }
    .empty { padding: 24px; color: #64748b; }
    .error { margin: 16px; border: 1px solid #fecaca; border-radius: 8px; background: #fff1f2; color: #991b1b; padding: 12px; white-space: pre-wrap; }
  </style>
</head>
<body>
  <header>
  ...
  <script>
    ...
    const indexedRows = payload.rows.map((row, index) => ({ row, rowNumber: index + 1 }));
